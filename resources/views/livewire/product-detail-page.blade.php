  ...
  <section class="overflow-hidden bg-white py-11 font-poppins dark:bg-gray-800">
    <div class="max-w-6xl px-4 py-4 mx-auto lg:py-8 md:px-6">
      <div class="flex flex-wrap -mx-4">
        <div class="w-full mb-8 md:w-1/3 md:mb-0" x-data="{ mainImage: '{{ asset('storage/'.$product->images[0]) }}' }">
          <div class="sticky top-0 z-50 overflow-hidden ">
            ...
            <div class="flex-wrap hidden md:flex ">
              ...
              @foreach ($product->images as $image)
                <div class="w-1/2 p-2 sm:w-1/4" x-on:click="mainImage='{{ asset('storage/'.$image) }}'" wire:key="{{ $product->id }}">
        ...
        <div class="w-full px-4 md:w-1/2 ">
          <div class="lg:pl-20">
              ...
              <div class="relative flex flex-row w-full h-10 mt-6 bg-transparent rounded-lg">
                ...
                class="flex items-center w-full font-semibold
                 text-center text-gray-700 placeholder-gray-700
                  bg-gray-300 outline-none dark:text-gray-400
                   dark:placeholder-gray-400 dark:bg-gray-900
                    focus:outline-none text-md hover:text-black"
                     placeholder="1">
            ...
            <div class="flex flex-wrap items-center gap-4">
              <button wire:click="addToCart({{$product->id}})" class="w-full p-4 cursor-pointer bg-blue-500 rounded-md lg:w-2/5 dark:text-gray-200 text-gray-50 hover:bg-blue-600 dark:bg-blue-500 dark:hover:bg-blue-700">
                <span wire:loading.remove wire:target="addToCart({{ $product->id }})">Add to cart</span>
                <span wire:loading.class="inline-block" class="hidden" wire:target="addToCart({{ $product->id }})">Adding...</span>
              </button>
            </div>
          </div>
        </div>
